...
if [ $# != 1 ]; then
	...
	aws elb create-load-balancer-policy --load-balancer-name loadBalancer --policy-name balancerPolicy --policy-type-name proxyProtPolicy --policy-attributes AttributeName=ProxyProtocol,AttributeValue=true
